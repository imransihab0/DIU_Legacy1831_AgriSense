...
| [x] | Produce a dated season calendar from land preparation to harvest, including sowing, fertilizer, irrigation, weed/pest checkpoints and harvest. | Prompt requires it; RAG includes crop calendars and fertilizer guidance. | Demo this with an explicitly chosen crop. Consider a structured calendar generator so dates/doses do not depend solely on LLM formatting. |
...
| [x] | Build and use a RAG knowledge base from public agronomy sources. | ChromaDB ingestion/search in `backend/app/rag/`; four source-attributed KB documents under `backend/data/kb/`. | Rebuild KB before demo and verify non-zero `/api/health` chunk count. Add direct source URLs/page references and retrieval tests. |
...
| [~] | Proactive weather-triggered advice that watches forecasts and adjusts plans. | Forecast-risk rules and SMS tool are specified in the prompt; no scheduled/background forecast monitor was found. | **High priority:** add a scheduler/polling job, persist planned task dates, and create/display alerts without waiting for a new chat turn. |
...
| [~] | Pest/disease risk from crop, growth stage and weather, with prevention/treatment and estimated cost. | Pest KB exists and the agent can retrieve it; no dedicated risk model/tool or treatment costing was found. | **High priority:** add a rules-based pest-risk tool using weather + stage and connect recommended treatments to seeded input prices. |
...
3. **P0 — Harden the season-plan output.** Replace prompt-only calendar/scheduler construction with structured validation or a deterministic plan tool; validate dates, all required checkpoints, and finance values before rendering.
