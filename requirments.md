...
| [x] | Build and use a RAG knowledge base from public agronomy sources. | ChromaDB over **19 source-attributed KB docs / 195 chunks**, incl. real gov extractions: BARC **FRG 2024**, SRDI **Soil Fertility Atlas 2020**, DAE **Agromet Advisory**, DAE **Plant Protection 2015**, BBS **Ag-Stats Yearbook 2024 yields**, **BRRI Modern Rice Cultivation** (varieties, seedbed, AWD, LCC, pest/harvest), plus a livestock guide. Each cites org/year. | Verified retrieval. A DeepSeek extraction of BARI Krishi Projukti Hatboi was REJECTED (fabricated/templated data — impossible rice yields, duplicated Rice/Potato tables). Now includes 8 division-wise DAM market price reports (real, dated, with month/year trend). |
...
| [x] | bdapps CaaS checkout: complete request/response, balance deduction and receipt flow. | Tested against the LIVE sandbox (app DIUAIH, 2026-07-24): **real `S1000` SMS and a real `S1000` direct-debit charge**. Fixes from live testing: `paymentInstrumentName` = `"Mobile Account"` (not `MobileAccount`), auto-capture of the masked `subscriberId` from `E1951` + retry, and a balance-skip bug. `balance/query`+`list/pi` 404 on the gateway (skipped). | Done & verified. To charge again, top up the test subscriber's sandbox balance (E1378 when empty). |
...
| [~] | Security and privacy for farmer/payment data. | bdapps passwords redacted in displayed payloads; `.env` gitignored (never committed); phone numbers masked via bdapps subscriberId. **Right-to-be-forgotten:** the farmer can ask the agent to delete their data → confirm-gated `clear_farm_data` wipes profile/plan/history. | Remaining: input/session-id validation, rate limits. |
